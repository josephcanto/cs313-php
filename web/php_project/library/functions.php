<?php    
    ...
    function buildPeopleList($people) {
        if(!empty($people)) {
            ...
            if($numFamily != 0 && $numFriends != 0) {
                ...
                $peopleList = "<h2 class='people-list-heading'>Family</h2><p class='notice'>Looks like you haven't added any family members yet.</p>" . $friends;
            }
        }
        return $peopleList;
    }
    ...
    function getEventsInfoByPersonId($personId) {
        $db = dbConnect();
        $sql = 'SELECT id, name, date, frequency, reminder, person_id FROM events WHERE person_id = :personId ORDER BY date';
        $stmt = $db->prepare($sql);
        $stmt->bindValue(':personId', $personId, PDO::PARAM_INT);
        $stmt->execute();
